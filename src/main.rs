extern crate cryptasm;

use cryptasm::{symmetriccipher, buffer, aes, blockmodes};
use cryptasm::buffer::{ReadBuffer, WriteBuffer, BufferResult};

// Encrypt a buffer with the given key and iv using
// AES-256/CBC/Pkcs encryption.
fn aes_encrypt(data: &[u8],
               key: &[u8],
               iv: &[u8])
...
fn aes_decrypt(encrypted_data: &[u8],
               key: &[u8],
               iv: &[u8])
               -> Result<Vec<u8>, symmetriccipher::SymmetricCipherError> {
    let mut decryptor =
...
pub extern "C" fn c_encrypt(message_ptr: *const u8,
    ...
    // println!("{} - {}", cipher_len, out.len());

    cipher.copy_from_slice(out.as_slice());
}

#[no_mangle]
pub extern "C" fn c_decrypt(message_ptr: *const u8,
                            message_len: usize,
                            key_ptr: *const u8,
                            iv_ptr: *const u8,
                            plain_ptr: *mut u8)
    ...
    let out = aes_decrypt(message, key, iv).ok().unwrap();

    let plain = unsafe { std::slice::from_raw_parts_mut(plain_ptr, out.len()) };

    plain.copy_from_slice(out.as_slice());
    out.len()
}
