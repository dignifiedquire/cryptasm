...
impl<P: BlockProcessor, X: PaddingProcessor> BlockEngine<P, X> {
    ...
    /// and output history.
    fn new_with_history(processor: P,
                        padding: X,
                        block_size: usize,
                        in_hist: Vec<u8>,
                        out_hist: Vec<u8>)
    ...
    fn fast_mode<R: ReadBuffer, W: WriteBuffer>(&mut self,
                                                input: &mut R,
                                                output: &mut W)
                                                -> BlockEngineState {
        fn has_next<R: ReadBuffer, W: WriteBuffer>(input: &mut R,
                                                   output: &mut W,
                                                   block_size: usize)
                                                   -> bool {
    ...
    fn process<R: ReadBuffer, W: WriteBuffer>(&mut self,
                                              input: &mut R,
                                              output: &mut W,
                                              eof: bool)
    ...
    fn process(&mut self, input: &[u8], output: &mut [u8]) {
        ...
        while i < len {
            if self.bytes.is_empty() {
                ...
                self.algo
                    .encrypt_block(&self.ctr[..], wb.take_remaining());
...
mod test {
    ...
    fn aes_ecb_no_padding_tests() -> Vec<EcbTest> {
        vec![EcbTest {
                 ...
                 cipher: vec![0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b, 0x88, 0x4c, 0xfa,
                              0x59, 0xca, 0x34, 0x2b, 0x2e, 0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a,
                              0x2c, 0x3b, 0x88, 0x4c, 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e],
             }]
    }
    ...
    fn aes_ecb_pkcs_padding_tests() -> Vec<EcbTest> {
        vec![EcbTest {
                 ...
                 cipher: vec![0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b, 0x88, 0x4c, 0xfa,
                              0x59, 0xca, 0x34, 0x2b, 0x2e, 0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a,
                              0x2c, 0x3b, 0x88, 0x4c, 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e, 0x01,
                              0x43, 0xdb, 0x63, 0xee, 0x66, 0xb0, 0xcd, 0xff, 0x9f, 0x69, 0x91,
                              0x76, 0x80, 0x15, 0x1e],
             },
             EcbTest {
                 key: repeat(0).take(16).collect(),
                 ...
                 cipher: vec![0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b, 0x88, 0x4c, 0xfa,
                              0x59, 0xca, 0x34, 0x2b, 0x2e, 0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a,
                              0x2c, 0x3b, 0x88, 0x4c, 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e, 0x7a,
                              0xdc, 0x99, 0xb2, 0x9e, 0x82, 0xb1, 0xb2, 0xb0, 0xa6, 0x5a, 0x38,
                              0xbc, 0x57, 0x8a, 0x01],
             }]
    }
    ...
    fn aes_cbc_no_padding_tests() -> Vec<CbcTest> {
        vec![CbcTest {
                 ...
                 cipher: vec![0x5e, 0x77, 0xe5, 0x9f, 0x8f, 0x85, 0x94, 0x34, 0x89, 0xa2, 0x41,
                              0x49, 0xc7, 0x5f, 0x4e, 0xc9, 0xe0, 0x9a, 0x77, 0x36, 0xfb, 0xc8,
                              0xb2, 0xdc, 0xb3, 0xfb, 0x9f, 0xc0, 0x31, 0x4c, 0xb0, 0xb1],
             }]
    }
    ...
    fn aes_cbc_pkcs_padding_tests() -> Vec<CbcTest> {
        vec![CbcTest {
                 ...
                 cipher: vec![0x5e, 0x77, 0xe5, 0x9f, 0x8f, 0x85, 0x94, 0x34, 0x89, 0xa2, 0x41,
                              0x49, 0xc7, 0x5f, 0x4e, 0xc9, 0xe0, 0x9a, 0x77, 0x36, 0xfb, 0xc8,
                              0xb2, 0xdc, 0xb3, 0xfb, 0x9f, 0xc0, 0x31, 0x4c, 0xb0, 0xb1, 0xa4,
                              0xc2, 0xe4, 0x62, 0xef, 0x7a, 0xe3, 0x7e, 0xef, 0x88, 0xf3, 0x27,
                              0xbd, 0x9c, 0xc8, 0x4d],
             },
             CbcTest {
                 key: repeat(1).take(16).collect(),
                 ...
                 cipher: vec![0x5e, 0x77, 0xe5, 0x9f, 0x8f, 0x85, 0x94, 0x34, 0x89, 0xa2, 0x41,
                              0x49, 0xc7, 0x5f, 0x4e, 0xc9, 0xe0, 0x9a, 0x77, 0x36, 0xfb, 0xc8,
                              0xb2, 0xdc, 0xb3, 0xfb, 0x9f, 0xc0, 0x31, 0x4c, 0xb0, 0xb1, 0x6c,
                              0x47, 0xcd, 0xec, 0xae, 0xbb, 0x1a, 0x65, 0x04, 0xd2, 0x32, 0x23,
                              0xa6, 0x8d, 0x4a, 0x65],
             }]
    }
    ...
    fn aes_ctr_tests() -> Vec<CtrTest> {
        vec![CtrTest {
                 ...
                 plain: repeat(2).take(33).collect(),
                 cipher: vec![0x64, 0x3e, 0x05, 0x19, 0x79, 0x78, 0xd7, 0x45, 0xa9, 0x10, 0x5f,
    ...
    fn run_full_test<T: CipherTest, E: Encryptor, D: Decryptor>(test: &T,
                                                                enc: &mut E,
                                                                dec: &mut D) {
    ...
    fn run_inc<OpFunc, NextInFunc, NextOutFunc>(input: &[u8],
                                                output: &mut [u8],
                                                mut op: OpFunc,
                                                mut next_in_len: NextInFunc,
                                                mut next_out_len: NextOutFunc,
                                                immediate_eof: bool)
        where OpFunc: FnMut(&mut RefReadBuffer, &mut RefWriteBuffer, bool)
                            -> Result<BufferResult, SymmetricCipherError>,
    ...
    fn run_inc1_test<T: CipherTest, E: Encryptor, D: Decryptor>(test: &T,
                                                                enc: &mut E,
                                                                dec: &mut D) {
        let mut cipher_out: Vec<u8> = repeat(0).take(test.get_cipher().len()).collect();
        run_inc(test.get_plain(),
                &mut cipher_out,
    ...
    fn run_rand_test<T, E, D, NewEncFunc, NewDecFunc>(test: &T,
    ...
    fn run_test<T, E, D, NewEncFunc, NewDecFunc>(test: &T,
                                                 mut new_enc: NewEncFunc,
                                                 mut new_dec: NewDecFunc)
        where T: CipherTest,
              E: Encryptor,
...
mod bench {
    use aessafe;
    use blockmodes::{EcbEncryptor, CbcEncryptor, CtrMode, CtrModeX8, NoPadding, PkcsPadding};
